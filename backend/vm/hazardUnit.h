...
public:
    ...
    bool DetectLoadUseHazard(uint8_t ex_rd, bool ex_mem_read,
                             uint8_t id_rs1, uint8_t id_rs2,
                             bool rs1_is_float, bool rs2_is_float,
                             bool ex_is_float) const;

    // Detect EX hazard: ALU result needed by next instruction
    bool DetectEXHazard(uint8_t ex_rd, bool ex_reg_write,
                        uint8_t id_rs1, uint8_t id_rs2) const;

    // Detect MEM hazard: MEM stage result needed by ID stage
    bool DetectMEMHazard(uint8_t mem_rd, bool mem_reg_write,
                         uint8_t id_rs1, uint8_t id_rs2) const;
};
